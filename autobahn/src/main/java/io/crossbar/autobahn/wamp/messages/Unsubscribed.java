...
public class Unsubscribed implements IMessage {
    ...
    private static final long SUBSCRIPTON_NULL = -1;
    ...
    public static Unsubscribed parse(List<Object> wmsg) {
        ...
        long subscription = SUBSCRIPTON_NULL;
        String reason = null;
        if (wmsg.size() > 2) {
    ...
    public List<Object> marshal() {
        ...
        if (subscription != SUBSCRIPTON_NULL || reason != null) {
            Map<String, Object> details = new HashMap<>();
            if (reason != null) {
                details.put("reason", reason);
            }
            if (subscription != SUBSCRIPTON_NULL) {
